...
// ✅ use hljs from npm + import a theme
import hljs from 'highlight.js'
import 'highlight.js/styles/nord.css'

// ✅ this must exist: frontend/src/config/webcontainer.js
...
function SyntaxHighlightedCode(props) {
  ...
  useEffect(() => {
    ...
    // support both "language-xxx" and "lang-xxx"
    const isCode = cls.includes('language-') || cls.includes('lang-')
    if (isCode) {
      hljs.highlightElement(ref.current)
      // allow re-highlighting if the node updates
  ...
  useEffect(() => {
    ...
    if (!webContainer) {
      getWebContainer().then(container => {
        setWebContainer(container) // can be null in production
        console.log(container ? 'WebContainer started' : 'WebContainer unavailable in production')
      })
    }
    ...
    receiveMessage('project-message', data => {
      ...
      if (data.sender._id == 'ai') {
        let parsed = {}
        try { parsed = JSON.parse(data.message) } catch { /* ignore */ }

        // only try to mount if container exists (dev) and fileTree provided
        if (webContainer && parsed.fileTree) {
          ...
          <div className="top flex justify-between w-full">
            ...
            <div className="actions flex gap-2">
              <button
                onClick={async () => {
                  if (!webContainer) {
                    console.warn('WebContainer not available in production')
                    return
                  }
